...
## [2026-09-21] feat | публикация на GitHub и самообновление (v1.1.0)

- Репозиторий https://github.com/m-bikko/ui-arsenal, MIT. README несёт
  промпт-установщик для ИИ-агента (EN + RU): ставит скилл в `~/.claude/skills`,
  проверяет тремя командами и дословно прописывает блок в глобальный `CLAUDE.md`.
- `scripts/update.py` + файл `VERSION` — самообновление перед запуском.
  Версия определяется сравнением semver с `raw.githubusercontent.../VERSION`,
  кеш проверки на 24 часа, таймаут 6 с.
- **Главное решение:** реестр при обновлении **сливается**, а не перезаписывается.
  `data/sources.json` — это пользовательское состояние: туда `add_source.py` пишет
  локально добавленные источники. Union по `id`, побеждает более свежий `verified`,
  локальные записи сохраняются всегда. Перед каждым апдейтом — полный бэкап в
  `~/.cache/ui-arsenal/`. См. [[pages/entities/sources-json|sources-json]].
- Мажорный бамп `--auto` не применяет: major означает, что съехала схема, и смотреть
  должен человек. Оффлайн и недоступный GitHub — выход 0, работа не блокируется.
- Проверено на симуляции старой установки 1.0.0 с локально добавленным источником:
  локальный источник уцелел, локально более свежая запись не затёрта, два новых
  апстримных подтянулись, бэкап на месте. Отдельно проверены отказ от мажорного
  апдейта, `--off`, `UI_ARSENAL_NO_UPDATE=1` и недоступный хост.
- Процесс релиза записан в [[../references/maintaining.md|maintaining.md]] §4b:
  без бампа `VERSION` установленные копии изменение не увидят.
